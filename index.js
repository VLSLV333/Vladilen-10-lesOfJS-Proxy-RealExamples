...
//         get: (obj, prop, receiver) => (prop in receiver ? obj[prop] : void 0) // void 0 = код поверне undefined; prop in receiver = чи залишився такий ключ в ітерації, після усіх наших фільтрацій?; obj[prop] = якщо залишився, покажіть велью за ключем
//     })
// }
...
// Optimization                                          // при додаванні нових об'єктів в наш масив ми відразу додаємо відповідний запис з потрібним айді в інший об'єкт index, для того щоб пізніше створити кастомну функцію, яка буде шукати наші об'єкти по айді
// const userData = [
...
// const withDefaultWalue = (target, defaultValue = 0) => {            // буде додавати дефолтне значення ключам, якщо вони не визначені
//     return new Proxy (target, {
//         get: (obj, prop) => (prop in obj ? obj[prop] : defaultValue)
//     })
// }

// const position = withDefaultWalue ({
//     x: 22,
//     y: 11
// }, 2)
...
const IndexedArray = new Proxy(Array, {
    construct(target, [args]) {
        const index = {}
        args.forEach( person => (index[person.id] = person))
        return new Proxy (target(...args), {
            get (arr, prop){
             switch(prop){
                case "push": return item =>{
